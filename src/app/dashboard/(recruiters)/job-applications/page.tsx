...
function JobFilterWithData({
  ...
  return <JobFilter jobs={data?.jobs || []} selectedJobId={selectedJobId} onChange={onChange} />;
}
...
export default function RecruiterJobApplicationsPage() {
  ...
  const columns: ColumnDef<JobApplication>[] = [
    ...
    },
    {
      ...
      header: 'Candidate',
      cell: ({ row }) => {
        return <div className="font-medium">{row.original.candidateName || 'Anonymous'}</div>;
      },
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        if (!interviewState) return <Badge variant="outline">Not Started</Badge>;
      ...
      cell: ({ row }) => {
        return (
          <Button
            variant="ghost"
            size="sm"
            onClick={() => router.push(`/dashboard/job-applications/${row.original._id}`)}
          >
  ...
  const { data: applications, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await fetch(`/api/applications/recruiter?${queryParams.toString()}`);
    ...
    <div className="space-y-6">
      ...
      <Card className="mb-6">
        <CardHeader className="pb-3">
          ...
          <CardDescription>Select a job to see related applications</CardDescription>
        </CardHeader>
        <CardContent>
          <JobFilterWithData selectedJobId={jobId} onChange={handleJobFilterChange} />
        </CardContent>
      </Card>
