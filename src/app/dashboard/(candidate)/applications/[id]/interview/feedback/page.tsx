...
export async function generateMetadata(): Promise<Metadata> {
  return {
    title: `Interview Feedback | HireBlue`,
  };
}
...
export default async function InterviewFeedbackPage(props: Props) {
  ...
  // Fetch data inside try/catch — keep JSX outside
  ...
  // TypeScript narrowing — after try/catch + notFound() these are always defined
          ...
          <CardContent>
            ...
            {interviewState.feedbackGeneratedAt && (
              <p className="text-xs text-muted-foreground mb-4 text-center">
                Feedback generated: {new Date(interviewState.feedbackGeneratedAt).toLocaleString()}
              </p>
            )}
